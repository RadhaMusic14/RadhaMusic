...
from RADHAMUSIC.utils.pastebin import VISHNUBIN
from RADHAMUSIC.utils.stream.queue import put_queue, put_queue_index
from RADHAMUSIC.utils.thumbnails import get_thumb
...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await VISHNU.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await VISHNU.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
